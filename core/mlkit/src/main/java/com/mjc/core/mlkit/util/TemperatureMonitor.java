package com.mjc.core.mlkit.util;
...
public final class TemperatureMonitor implements SensorEventListener {
  ...
  private final Map<String, Float> sensorReadingsCelsius = new HashMap<>();
  ...
  public Map<String, Float> getSensorReadingsCelsius() {
    return sensorReadingsCelsius;
  }
  ...
  public void logTemperature() {
    for (Map.Entry<String, Float> entry : sensorReadingsCelsius.entrySet()) {
      ...
      Log.i(TAG, String.format(Locale.US, "%s:\t%.1fC\t%.1dF", entry.getKey(), tempC, tempF));
    }
  }
